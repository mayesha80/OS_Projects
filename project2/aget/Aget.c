...
void startHTTP(struct request *req)
{
    ...
    Log("\033[34mDownloading %s (%d bytes) from site %s(%s:%d).\nNumber of Threads: %d\033[0m",
...
retry:
	...
	Log("\033[31mDownload completed, job completed in %d seconds. (%d Kb/sec)\033[0m",
...
void startFTP(struct request *req)
{
	...
	wthread = (struct thread_data *)calloc(nthreads, sizeof(struct thread_data));
	Log("\033[95mResuming download %s (%d bytes) from site %s(%s:%d).\nNumber of Threads: %d\033[0m",
    req->url, req->clength, req->host, req->ip, req->port, nthreads);
...
retry:
	...
	if (nok == nthreads) {
        ...
        if ((diff_sec = t_finish - t_start) == 0)
		diff_sec = 1;   /* Avoid division by zero */
	Log("\033[93mDownload completed, job completed in %d seconds. (%d Kb/sec)\033[0m",
    diff_sec, ((req->clength - h->bwritten) / diff_sec) / 1024);

Log("\033[32mTotal download time: %d seconds. Overall speed: %d Kb/sec\033[0m",
