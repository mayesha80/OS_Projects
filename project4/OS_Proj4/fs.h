...
typedef struct { 
    ...
    int directBlock[15]; 
    int indirectBlock; 
    int linkCount;      // existing hardlink field 
    int owner;          // new owner ID
    int group;          // new group ID
    struct timeval created;     // new creation time
    struct timeval lastAccess;  // new last access time
    char padding[4];   // Adjust padding to maintain structure size 
} Inode;
